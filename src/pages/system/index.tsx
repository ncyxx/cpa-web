...
export function SystemPage() {
  ...
  const triggerUpdate = useCallback(async () => {
    ...
    try {
      const result = await configApi.triggerPanelUpdate()
      if (result.updated) {
        setUpdateMessage('更新成功！请刷新页面')
        setHasUpdate(false)
        setLatestVersion(result.latest_version)
      } else {
        setUpdateMessage(result.message || '已是最新版本')
      }
    } catch (err: any) {
      setUpdateMessage(`更新失败: ${err?.message || '未知错误'}`)
    } finally {
            ...
            <div className="mt-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <button 
                  ...
                  disabled={versionLoading || updateLoading}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-green-600 mt-0.5">点击更新按钮自动下载安装</p>
                      </div>
                    </div>
                    <button 
                      onClick={triggerUpdate}
                      disabled={updateLoading}
                      className="px-4 py-2 rounded-xl bg-green-500 text-white text-sm font-medium hover:bg-green-600 transition-colors cursor-pointer disabled:opacity-50 flex items-center gap-2"
                    >
                      {updateLoading ? (
                        <>
              ...
              {updateMessage && (
                <div className={`mt-4 p-4 rounded-xl flex items-center justify-between ${
                  updateMessage.includes('成功') ? 'bg-green-50 border border-green-100' : 
                  updateMessage.includes('失败') ? 'bg-red-50 border border-red-100' : 
                  'bg-gray-50 border border-gray-100'
                }`}>
                  <span className={`text-sm font-medium ${
                    updateMessage.includes('成功') ? 'text-green-700' : 
                    updateMessage.includes('失败') ? 'text-red-700' : 
                    'text-gray-700'
                  }`}>
                    {updateMessage}
                  </span>
                  {updateMessage.includes('成功') && (
                    <button 
                      ...
                      className="px-4 py-2 rounded-xl bg-green-500 text-white text-sm font-medium hover:bg-green-600 transition-colors cursor-pointer"
                    >
                      刷新页面
                    </button>
                  )}
